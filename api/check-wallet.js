export default async function handler(req, res) {
  ...
  // Using Official Base RPC (More stable than Alchemy Demo)
  ...
  try {
    ...
    const ethVolume = txCount * 0.015; // Adjusted estimate
    const usdcVolume = usdcBalance * 2.5;
    
    const points = (txCount * 1.2) + (ethVolume * 15) + (usdcBalance > 50 ? 50 : 0) + (daysActive * 2);
    ...
    res.status(200).json({
      ...
      calculation: {
        ...
        estimatedAllocation: allocation.toLocaleString()
      }
    });

  } catch (error) {
    console.error("RPC Error:", error.message);
    res.status(500).json({ error: "Node Provider Error", details: error.message });
  }
}
